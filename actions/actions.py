...
class ActionStrategy(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        d_send={"intent": str(intent_name), "data": {"id": int(ent)}}
        print(d_send)
...
def get_list_string (tracker: Tracker) -> List:
    ...
    for x in task_string3:
        x_string=str(x)
        if (len(x_string)>1):
            task_string4.append(x_string[1])
        else:
...
class ActionEndedMeeting(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        d_intent={"intent":"ended_meeting","data":{"id":int(ent_idreunion),"fecha_start": str(fecha_inicio),"fecha_ended":str(fecha_fin)}}
